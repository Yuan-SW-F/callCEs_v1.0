...
#PR="H=2000 X=9400 L=3000 K=2200 --format=axt" #xhj
PR="--ambiguous=iupac --chain --notransition H=2000 Y=3000 L=3000 K=2200 --format=axt --gfextend" #old
#PR="--gapped --ambiguous=n --step=10 --strand=both --masking=10 --maxwordcount=500 --identity=70..100 --format=axt" #gold fish
...
for i in \`ls $QN.dir/* | grep -v axt \`;do 
	if [ ! -s \$i-$RM.axt ];then
		lastz $1[multiple] \$i[multiple] $PR > \$i-$RM.axt
	fi
	if [ -s \$i-$RM.axt ];then
		axtSort \$i-$RM.axt \$i-$RM.axt.sorted
		axtBest \$i-$RM.axt.sorted all \$i-$RM.axt.sorted.best
		while [ ! -s \$i-$RM.axt.sorted.best ];do
			lastz $1[multiple] \$i[multiple] $PR > \$i-$RM.axt
			axtSort \$i-$RM.axt \$i-$RM.axt.sorted
			axtBest \$i-$RM.axt.sorted all \$i-$RM.axt.sorted.best
		done
		...
		rm \$i-$RM.axt
		rm \$i-$RM.axt.sorted
		mv \$i-$RM.axt.sorted.best \$i-$RM.axt
	fi
done 
cat $QN.dir/*axt > $RM-$QM.axt
